...
const imagePopupView = {
	init: function(){
		...
		this.outsideArea = document.querySelector('.popuparea');
		this.closeBtn.addEventListener('click',function(){
			imagePopupView.hide();
		});
		this.outsideArea.addEventListener('click',function(){
	...
	render: function(){
		...
		this.rotate = document.querySelector('.toolbar .btnRotate');
		let title = document.getElementsByClassName("title")[0];
		title.innerHTML=image.name;
		this.rotate.addEventListener('click',function(){
			imagePopupView.imageRotate();
		});
